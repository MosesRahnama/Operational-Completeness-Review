...
>           simp [kappa]
>         have hkR : kappa (app s (recΔ b s (delta m))) =
>           Nat.max (Nat.max (kappa b) (kappa s)) (kappa m) + 1 := by
...
>           le_trans (Nat.le_max_right _ _) (Nat.le_max_left _ _)
>         have h_rhs : kappa (app s (recΔ b s void)) = base := by
...
>         have hs_le : kappa s ≤ base :=
>           le_trans (Nat.le_max_right _ _) (Nat.le_max_left _ _)
>         have h_rhs : kappa (app s (recΔ b s (integrate t))) = base := by
...
>         have hs_le : kappa s ≤ base :=
>           le_trans (Nat.le_max_right _ _) (Nat.le_max_left _ _)
>         have h_rhs : kappa (app s (recΔ b s (merge a c))) = base := by
...
>           simp [kappa]
>         have h_rhs : kappa (app s (recΔ b s (recΔ b' s' n'))) = base := by
...
>         have hs_le : kappa s ≤ base :=
>           le_trans (Nat.le_max_right _ _) (Nat.le_max_left _ _)
>         have h_rhs : kappa (app s (recΔ b s (eqW a c))) = base := by
