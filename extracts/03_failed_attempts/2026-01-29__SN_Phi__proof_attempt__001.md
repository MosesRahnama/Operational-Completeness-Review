...
>   exact this
> 
> /-- rec_succ: app s (recΔ b s n) < recΔ b s (delta n). -/
> theorem phi_merge_lt_rec (b s n : Trace) :
>   phi (app s (recΔ b s n)) < phi (recΔ b s (delta n)) := by
...
>     simpa [phi, hA]
>   have : phi (app s (recΔ b s n)) < A := by
>     have eq_mu : phi (app s (recΔ b s n)) = (omega0 ^ (3 : Ordinal)) * (phi s + 1)
